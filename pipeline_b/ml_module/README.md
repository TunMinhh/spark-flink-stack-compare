# Pipeline B Machine Learning Module

This module trains and serves the AttentionLSTM anomaly model for the Flink /
Iceberg pipeline.

```text
Silver Iceberg intraday tables
  heart_rate_intraday
  hrv_intraday
  breathing_intraday
...
-> Gold Iceberg ai_intraday_insights
```
...
| File | Purpose |
| --- | --- |
| `train.py` | Loads Silver intraday Iceberg data, trains AttentionLSTM, logs to MLflow, and writes Gold AI insights. |
...
Training reads these Silver Iceberg tables from `ICEBERG_SILVER_NS`:
...
Training writes Gold Iceberg table:
...
Start the default stack from `pipeline_b`:
